...
sub web_acronym {
    ...
    my $objHTTP = get_file_contents("http://www.all-acronyms.com/" . $content);
...
sub web_acronymdb {
    ...
    my $objHTTP = get_file_contents("http://www.acronymdb.com/acronym/" . $content);
    if (my @matches =  ($objHTTP =~ m/alt=['"]Definition of acronym [^:]+: ([^'"]+)/msg)) {
        my ($output, $first, $max) = ("", 1, 5);
        for (@matches) {
            $output .= ", " unless $first;
            $output .= $_;
            $first = 0;
